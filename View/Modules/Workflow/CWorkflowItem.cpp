...
CWorkflowItem::CWorkflowItem(QGraphicsItem *parent) : QGraphicsPathItem(parent)
{
...
CWorkflowItem::CWorkflowItem(QString name, WorkflowVertex id, QGraphicsItem *parent) : QGraphicsPathItem(parent)
{
    m_name = name;
    m_id = id;
    setFlag(QGraphicsItem::ItemIsMovable);
...
CWorkflowItem::~CWorkflowItem()
{
    delete m_pHeaderTextItem;
}

void CWorkflowItem::setHeaderColor(QColor bgColor, QColor textColor)
{
    m_headerBckColor = bgColor;
    m_headerTextColor = textColor;
}

void CWorkflowItem::setBodyColor(QColor color)
{
    m_bodyBckColor = color;
}

void CWorkflowItem::setLineColor(QColor lineColor, QColor lineSelectedColor)
{
    m_lineColor = lineColor;
    m_lineSelectedColor = lineSelectedColor;
}

void CWorkflowItem::setStatus(CWorkflowTask::State status)
{
    m_status = status;
    if(m_status == CWorkflowTask::State::VALIDATE)
        m_statusMsg = QObject::tr("Success");
    else if(m_status == CWorkflowTask::State::UNDONE)
...
void CWorkflowItem::setStatusMsg(const QString& msg)
{
    m_statusMsg = msg;
}

void CWorkflowItem::setInputPortAssigned(int index, bool bAssigned)
{
...
void CWorkflowItem::setSize(QSize itemSize)
{
...
void CWorkflowItem::setActionButtonSize(int size)
{
    m_actionButtonSize = size;
}

void CWorkflowItem::setPortRadius(float radius)
{
    m_portRadius = radius;
}

void CWorkflowItem::setIOInfo(const CDataInfoPtr &info, int index, bool bInput)
{
    if(bInput)
    {
...
WorkflowVertex CWorkflowItem::getId() const
{
    return m_id;
}

int CWorkflowItem::type() const
{
    return Type;
}

int CWorkflowItem::getInputPortCount() const
{
    return m_inputPorts.size();
}

int CWorkflowItem::getOutputPortCount() const
{
    return m_outputPorts.size();
}

int CWorkflowItem::getInputConnectionCount() const
{
    int nb = 0;
...
int CWorkflowItem::getOutputConnectionCount() const
{
    int nb = 0;
...
CWorkflowPortItem *CWorkflowItem::getInputPort(int index)
{
    if(index >= 0 && index < m_inputPorts.size())
...
CWorkflowPortItem *CWorkflowItem::getOutputPort(int index)
{
    if(index >= 0 && index < m_outputPorts.size())
...
QVector<CWorkflowConnection*> CWorkflowItem::getConnections() const
{
    QVector<CWorkflowConnection*> connections;
...
bool CWorkflowItem::isConnectedTo(const WorkflowVertex &id) const
{
    for(int i=0; i < m_inputPorts.size(); ++i)
    {
...
void CWorkflowItem::addInputPort(IODataType type, const QColor& color)
{
    auto pPort = new CWorkflowPortItem(m_inputPorts.size(), true, m_id, type, color, this);
...
void CWorkflowItem::addOutputPort(IODataType type, const QColor& color)
{
    auto pPort = new CWorkflowPortItem(m_outputPorts.size(), false, m_id, type, color, this);
    pPort->setRadius(m_portRadius);
    float radius = pPort->getRadius();
    qreal top = m_headerHeight + (2 * radius);
...
QPushButton* CWorkflowItem::addActionButton(CWorkflowTask::ActionFlag flag, const QString& iconUncheckedPath, const QString& iconCheckedPath, const QString& text)
{
    //Style sheet
...
void CWorkflowItem::removeInputPort(int index)
{
    if(index < m_inputPorts.size())
    {
...
void CWorkflowItem::removeOutputPort(int index)
{
...
void CWorkflowItem::paintStatus(QPainter* painter, QRect rect)
{
    painter->save();
    ...
    if(m_status == CWorkflowTask::State::UNDONE)
    {
        painter->setPen(QPen(m_lineColor, 3));
        painter->setBrush(Qt::transparent);
    }
    else if(m_status == CWorkflowTask::State::VALIDATE)
    {
        painter->setPen(QPen(m_lineColor, 3));
...
void CWorkflowItem::updateInputPorts(const WorkflowTaskPtr &pTask)
{
    ...
    for(int i=0; i<std::min(itemPortNb, taskPortNb); ++i)
    {
        ...
        m_inputPorts[i]->setColor(Utils::Workflow::getPortColor(type));
    }
    ...
    if(itemPortNb < taskPortNb)
    {
        ...
        {
            IODataType type = pTask->getInputDataType(i);
            auto color = Utils::Workflow::getPortColor(type);
...
void CWorkflowItem::updateOutputPorts(const WorkflowTaskPtr &pTask)
{
    ...
    for(int i=0; i<std::min(itemPortNb, taskPortNb); ++i)
    {
        m_outputPorts[i]->setColor(Utils::Workflow::getPortColor(pTask->getOutputDataType(i)));
...
void CWorkflowItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
void CWorkflowItem::updateConnections()
{
    for(int i=0; i < m_inputPorts.size(); ++i)
...
void CWorkflowItem::updatePorts(const WorkflowTaskPtr &pTask)
{
    assert(pTask);
    //Update inputs
...
void CWorkflowItem::updateActions(const std::map<CWorkflowTask::ActionFlag, bool> &flags)
{
    //Remove widgets not wanted anymore
    for(auto it=m_actionWidgets.begin(); it!=m_actionWidgets.end();)
    {
        auto itFlag = flags.find(static_cast<CWorkflowTask::ActionFlag>(it.key()));
        if(itFlag == flags.end())
        {
    ...
    for(auto it=flags.begin(); it!=flags.end(); ++it)
    {
        ...
        else
        {
            ...
            switch(flag)
            {
                case CWorkflowTask::ActionFlag::APPLY_VOLUME:
                ...
                case CWorkflowTask::ActionFlag::OUTPUT_AUTO_EXPORT:
                    pBtn = addActionButton(flag,
...
void CWorkflowItem::updateActionButtonsPos()
{
...
void CWorkflowItem::updateItem()
{
    initHeader();
    initBody();
}

void CWorkflowItem::prepareRemove()
{
    //Workaround to prevent QT bug when removing from the scene a QGraphicsItem with QGraphicsDropShadowEffect
    prepareGeometryChange();
}

void CWorkflowItem::hoverEnterEvent(QGraphicsSceneHoverEvent *event)
{
    setCursor(Qt::ArrowCursor);
    QGraphicsItem::hoverEnterEvent(event);
}

void CWorkflowItem::hoverMoveEvent(QGraphicsSceneHoverEvent* event)
{
