...
CWorkflowIOArea::CWorkflowIOArea(QString label, bool bInput, QGraphicsItem *parent) : CWorkflowLabelArea(label, parent)
{
    m_bInput = bInput;
    m_id = boost::graph_traits<WorkflowGraph>::null_vertex();
    setAcceptHoverEvents(true);
    setZValue(1.0);
}

void CWorkflowIOArea::setTaskId(const WorkflowVertex &id)
{
    m_id = id;
}

void CWorkflowIOArea::setProjectDataProxyModel(CProjectDataProxyModel *pModel)
{
    if(m_pInputChoiceDlg)
        m_pInputChoiceDlg->setModel(pModel);
}

void CWorkflowIOArea::setPortInfo(const CDataInfoPtr &info, int index)
{
...
int CWorkflowIOArea::type() const
{
    return Type;
}

int CWorkflowIOArea::getPortCount() const
{
    return m_ports.size();
}

int CWorkflowIOArea::getConnectionCount() const
{
    int nb = 0;
...
CWorkflowPortItem *CWorkflowIOArea::getPort(int index) const
{
    if(index >= 0 && index < m_ports.size())
...
bool CWorkflowIOArea::isInput() const
{
    return m_bInput;
}

void CWorkflowIOArea::activateActions(bool bActivate)
{
    m_bActionsActivated = bActivate;
}

void CWorkflowIOArea::addPort(const WorkflowTaskIOPtr& inputPtr, bool isBatch)
{
    ...
    if(inputPtr != nullptr)
    {
        ...
        portColor = Utils::Workflow::getPortColor(type);
    }

    auto pPort = new CWorkflowPortItem(m_ports.size(), bInput, m_id, type, portColor, this);
...
void CWorkflowIOArea::updatePort(int index, const WorkflowTaskIOPtr &inputPtr, bool isBatch)
{
    if(index < 0 || index >= m_ports.size())
        return;
...
void CWorkflowIOArea::removePort(int index)
{
...
void CWorkflowIOArea::onAddPort()
{
    createInputTypeDlg();

    if(m_pInputChoiceDlg->exec() == QDialog::Accepted)
    {
        CWorkflowInput input = m_pInputChoiceDlg->getInput();
        if(input.isValid())
        {
            auto pScene = static_cast<CWorkflowScene*>(scene());
...
void CWorkflowIOArea::hoverEnterEvent(QGraphicsSceneHoverEvent *event)
{
    Q_UNUSED(event);
...
void CWorkflowIOArea::hoverLeaveEvent(QGraphicsSceneHoverEvent *event)
{
    Q_UNUSED(event);
...
QGraphicsProxyWidget* CWorkflowIOArea::createThumbnail(const std::shared_ptr<CWorkflowTaskIO> &inputPtr, bool isBatch)
{
    auto pixmap = getThumbnailIcon(inputPtr, isBatch);
...
void CWorkflowIOArea::createInputTypeDlg()
{
    m_pInputChoiceDlg = new CWorkflowInputTypeDlg;
    connect(m_pInputChoiceDlg, &CWorkflowInputTypeDlg::doQueryProjectDataProxyModel, [&](const TreeItemType& inputType, const std::vector<DataDimension>& filters)
    {
        auto pScene = static_cast<CWorkflowScene*>(scene());
...
QPointF CWorkflowIOArea::getNextPortCenter() const
{
    QPointF center;
    QRectF rect = boundingRect();
    qreal portAreaHeight = m_thumbnailSize.height() + (2 * m_portSpacing);
...
QPixmap CWorkflowIOArea::getThumbnailIcon(const std::shared_ptr<CWorkflowTaskIO> &inputPtr, bool isBatch)
{
    if(inputPtr == nullptr)
...
QPixmap CWorkflowIOArea::getImageIcon(const std::shared_ptr<CImageIO> &inputImgPtr, bool isBatch)
{
...
int CWorkflowIOArea::getThumbnailIndex(QGraphicsProxyWidget *pThumbnail)
{
...
QString CWorkflowIOArea::createSvgThumbnails(QPixmap pixmap, int index)
{
...
void CWorkflowIOArea::changeInput(int index)
{
    createInputTypeDlg();
...
void CWorkflowIOArea::deleteInput(int index)
{
    auto pScene = static_cast<CWorkflowScene*>(scene());
    assert(pScene);
...
void CWorkflowIOArea::hideAddPortWidget()
{
    m_pAddPortWidget->hide();
}

#include "moc_CWorkflowIOArea.cpp"
