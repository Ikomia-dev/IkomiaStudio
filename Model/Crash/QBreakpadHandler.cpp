...
QBreakpadHandler::QBreakpadHandler() :
    d(new QBreakpadHandlerPrivate()), m_pSender(new QBreakpadHttpUploader)
{
    connect(m_pSender, &QBreakpadHttpUploader::uploadProcess, [&](qint64 sent, qint64 total){ emit doUploadProgress(sent, total); });
    connect(m_pSender, &QBreakpadHttpUploader::finished, [&]{ emit doFinish(); });
}
...
QBreakpadHttpUploader* QBreakpadHandler::getUploader()
{
    return m_pSender;
}
...
void QBreakpadHandler::sendDumps()
{
    if(!d->dumpPath.isNull() && !d->dumpPath.isEmpty())
    {
        ...
        foreach(QString itDmpFileName, dumpFiles) {
            ...
            m_pSender->uploadDump(d->dumpPath + "/" + itDmpFileName);
        }
    }
}
