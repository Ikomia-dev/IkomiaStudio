...
CMat CVideoPlayer::getImage(int pos)
{
...
void CVideoManager::updateVideoPos(const QModelIndex& modelIndex, size_t index, int pos)
{
    auto pPlayer = getPlayer(modelIndex);
    if(!pPlayer)
        return;
...
int CVideoManager::getCurrentPos(const QModelIndex& index)
{
    // Get info generated by getImage()
    auto pPlayer = getPlayer(index);
    if(!pPlayer)
...
CMat CVideoManager::getVideoImage(const QModelIndex& index, int pos)
{
    auto pPlayer = getPlayer(index);
    if(!pPlayer)
        return CMat();
